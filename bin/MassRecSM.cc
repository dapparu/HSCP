...
int main(int argc,char** argv)
{
    ...
    for(int i=1;i<=_hdEdxRec->GetNbinsX();i++)
    {
        ...
        if(i%divider==0)
        {
            ...
            if(_hdEdxProjY->GetEntries()>0)
            {
                ...
                hrec->SetBinContent(_hdEdxRec->GetBinLowEdge(fitresX->Parameter(1)),fit_res->Parameter(1));
    ...
    TFitResultPtr fit = hrec->Fit("InvSquare","QRS","",0,5);
