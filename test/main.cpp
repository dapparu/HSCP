...
// ------------------------------------------
    ...
    hPt->Draw();
    cPt->SaveAs("./test2.pdf");
    
    TCanvas* c1 = new TCanvas("c1","c1",700,400);

    TFitResultPtr r = prof1->Fit("pol1","S");
    TFitResultPtr fitGaus = h3->Fit("gaus","S");



    float factor = 1+fitGaus->Parameter(1);

    TH1F* calibh1 = new TH1F("","calibh1",200,-1,1);
    TH2F* calibh2 = new TH2F("","calibh2",300,0,900,300,0,900);
    TProfile* calibProf = new TProfile("","calibProfile",50,0,900,"");
    
    
    

    TEfficiency* rat254 = new TEfficiency(*hLayerSat254,*hLayerLabel);
    TEfficiency* rat255 = new TEfficiency(*hLayerSat255,*hLayerLabel);

    TEfficiency* PtSat254 = new TEfficiency(*hPt_sat254,*hPt);
    TEfficiency* PtSat255 = new TEfficiency(*hPtsat255,*hPt);


    TFile* fileout = new TFile("../results/res.root","RECREATE");
   

    c1->cd();
    h2->Draw();
    h2->SetDrawOption("COLZ");
    h2->SetTitle("Eloss vs. charge");
    h2->GetXaxis()->SetTitle("eloss");
    h2->GetYaxis()->SetTitle("charge");
    //prof1->SetLineWidth(1);
    //prof1->SetLineColor(2);
    //prof1->Draw("SAME");
    c1->SaveAs("../results/QvEloss.pdf");
    c1->cd();
    h3->Draw();
    h3->SetTitle("(eloss-charge)/charge");
    h3->GetXaxis()->SetTitle("(eloss-charge)/charge");
    c1->SaveAs("../results/diffrel_ChargeEloss.pdf");
    c1->cd();
    h1->Draw();
    h1->SetTitle("eloss/charge");
    h1->GetXaxis()->SetTitle("eloss/charge");
    c1->SaveAs("../results/elossdividecharge.pdf");
    c1->cd();
    prof1->Draw();
    c1->SaveAs("../results/tprofile.pdf");
    c1->cd();
    calibProf->Draw();
    c1->SaveAs("../results/calibProfile.pdf");
    c1->cd();
    calibh1->Draw();
    c1->SaveAs("../results/calibDiffRel.pdf");
    c1->cd();
    calibh2->Draw();
    calibh2->SetDrawOption("COLZ");
    calibh2->SetTitle("Eloss vs. charge");
    calibh2->GetXaxis()->SetTitle("eloss");
    calibh2->GetYaxis()->SetTitle("charge");
    //prof1->SetLineWidth(1);
    //prof1->SetLineColor(2);
    //prof1->Draw("SAME");
    c1->SaveAs("../results/CalibQvEloss.pdf");
    c1->cd();
    hLayer->Draw();
    c1->SaveAs("../results/layer.pdf");
    hLayerLabel->Draw();
    c1->SaveAs("../results/layerLabel.pdf");
    SetHistoLabel(c1,hLayerLabel);
    hLayerSat254->SetLineColor(2);
    hLayerSat254->Draw("SAME");
    hLayerSat255->SetLineColor(3);
    hLayerSat255->Draw("SAME");
    c1->SaveAs("../results/layerlabel.pdf");

    c1->cd();
    hPt->Draw();
    c1->SetLogy(0);
    hPt_sat254->SetLineColor(2);
    hPtsat255->SetLineColor(3);
    hPt_sat254->Draw("SAME");
    hPtsat255->Draw("SAME");
    c1->SaveAs("../results/hPt.pdf");
    //SetHistoLabel(c1,hRatio);


    
    SetHistoLabel(c1,hEmpty);
    rat255->SetLineColor(2);
    c1->cd();
    //c1->SetLogy();
    hEmpty->GetYaxis()->SetRangeUser(pow(10,-3),0.3);
    rat254->Draw("SAME");
    rat255->Draw("SAME");
    c1->SaveAs("../results/ratio_sat.pdf");

    TCanvas* c2 = new TCanvas("c2","c2",700,400);

    PtSat255->SetLineColor(2);
    c2->cd();
    c2->SetLogy(0);
    PtSat254->Draw();
    PtSat255->Draw("SAME");
    c2->SaveAs("../results/pt_sat.pdf");

    c1->cd();
    c1->SetLogy(0);
    hNClusterPerTrack->Draw();
    hNClusterSat254->SetLineColor(2);
    hNClusterSat254->Draw("SAME");
    hNClusterSat255->SetLineColor(3);
    hNClusterSat255->Draw("SAME");
    hNClusterPerTrack->GetYaxis()->SetRangeUser(0,11000);
    c1->SaveAs("../results/NClusterPerTrack.pdf");
    ...
    float ratio254 =(float)ClustSat254/(float)nTotClust;
    float ratio255 =(float)ClustSat255/(float)nTotClust;

    cout<<"TotClust "<<nTotClust<<endl;
    cout<<"Sat254 "<<ClustSat254<<endl;
    cout<<"Sat255 "<<ClustSat255<<endl;
    cout<<"ratio254 "<<ratio254<<endl;
    cout<<"ratio255 "<<ratio255<<endl;*/
